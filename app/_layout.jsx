...
export const dbName = 'estnewv32.db';
  ...
  const createDbIfNeeded = async (db) => {
    ...
    if(foodCheck[0].rowCount < 1) {
      ...
      DEFAULT_FOODS.forEach(item => {
        db.runAsync(
          'INSERT INTO foods (name, id, calories_per_serving, serving_size, protein_per_serving, carbs_per_serving, fat_per_serving, times_selected) VALUES (?,?,?,?,?,?,?,?)',
          [item.name, item.id, item.calories_per_serving, item.serving_size, item.protein_per_serving, item.carbs_per_serving, item.fat_per_serving, item.times_selected]
        )
      })
    }
